...
def demo(video, design, timestep, height): # todo: method name should be more descriptive
    ...
    files = ['video', 'design']
    df_files = pd.DataFrame([meta[k] for k in files], index=files, columns=['path'])
    coordinates = ['pt' + str(i) for i in range(4)]
    df_coordinates = pd.DataFrame(meta['coordinates'], index=coordinates, columns=['x', 'y'])
    df_transform = pd.DataFrame(meta['transform'])
    df_colors = pd.DataFrame(meta['colors']).transpose()

    df_files.to_excel(writer, header=False, sheet_name='metadata')
    df_coordinates.to_excel(writer, startrow=3, index=True, sheet_name='metadata')
    df_transform.to_excel(writer, startrow=3, startcol=4, sheet_name='metadata')
    df_colors.to_excel(writer, startrow=9, sheet_name='metadata')
